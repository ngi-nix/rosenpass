  ...
  outputs = {
    ...
    nixpkgs.lib.foldl (a: b: nixpkgs.lib.recursiveUpdate a b) {} [
          ...
          system: let
            ...
            pkgs = import nixpkgs {
              ...
              overlays = [
                (
                  final: prev: {
                    iproute2 = prev.iproute2.overrideAttrs (old: let
            ...
            in
              naersk.buildPackage
              {
                ...
                "CARGO_TARGET_${shout target}_LINKER" = let
                  inherit (p.stdenv) cc;
              ...
              rec {
                ...
                release-package = let
                  ...
                  package =
                    ...
                    then packages.rosenpass-static
                    else packages.rosenpass;
                  oci-image =
                    if pkgs.hostPlatform.isLinux
                    then packages.rosenpass-static-oci-image
      ...
      in {
        ...
        checks = {
          cargo-fmt =
            pkgs.runCommand "check-cargo-fmt"
          ...
          nixpkgs-fmt =
            ...
            '';
          prettier-check =
            pkgs.runCommand "check-with-prettier"
